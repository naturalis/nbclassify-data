---
title: "Script for 500 butterfly clusters set 2"
author: "Wim"
date: "12 mei 2016"
output:
  html_document:
...
##actual begin here
````{r load data}
setwd("~/Leiden/2015-2016/Stage/PCA spul/Set 2/500") #change as necessary
#import data from server here!
tsv.data.orig <- read.csv("BagOfWords500_set2.csv",sep = ";")
...
bartlett.test(tsv.data.orig[11:510]) #check for variances, skipping first ten (because they're factors). if p<0.05 variance differs greatly and we need to set cor to TRUE in the prcomp.
...
summary(mv.dim.orig) #importance of components of PCA.  the first x PCA's are 75% of all variance. Here it's the first 84.
...
mv.dim.orig$rotation[,1:84] #check here what are the most important components for the different principle components. Gonna have to find a way to make this more readable. This is essientially what we want though. Afterwards check what the most important features are and check below for correlations.
...
rcorr(as.matrix(tsv.matrix[,11:510]), type=c("spearman")) #shows data-matrix of correlations between features (mcor) AND checks for p-value of correlations. if x<0.05, check if the correlation is high. If yes, you might wanna remove one of the two values.
...
writeWebGL(dir="C:\\Users\\wim_v\\Documents\\Leiden\\2015-2016\\Stage\\PCA spul\\Set 2\\500", width=700, filename=file.path("C:\\Users\\wim_v\\Documents\\Leiden\\2015-2016\\Stage\\PCA spul\\Set 2\\500", "3dplot_500_2.html")); browseURL(file.path("C:\\Users\\wim_v\\Documents\\Leiden\\2015-2016\\Stage\\PCA spul\\Set 2\\500", '3dplot_500_2.html'))
````
