...
tsv.data.orig <- read.csv("BagOfWords150_soorten.csv",sep = ";")
...
summary(tsv.data.orig[21:30,21:30])
bartlett.test(tsv.data.orig[9:158]) #check for variances, skipping first two (because they're factors). if p<0.05 variance differs greatly
...
mv.dim.orig <- prcomp(tsv.data.orig[,-1:-8], loadings=TRUE, scale=TRUE, scores=TRUE, cor=TRUE) #creating pca dataset
...
mcor <- cor(tsv.data.orig[,-1:-8], method = c("spearman")) #creates a correlation-matrix and removes the first two factorial values.
...
corrgram(tsv.data.orig[,8:22], lower.panel = panel.ellipse,upper.panel = panel.conf,diag.panel = panel.density, cor.method = "spearman" )
...
pairs(mv.dim.orig$x[, 1:18], pch = 21, col=species) #compares relationships between PC's. 9 maximum.
...
text(mv.dim.orig$x[, 1], mv.dim.orig$x[, 2], as.character(ID), cex = 0.9, pos = 3, col = "blue") #adds individuals to datapoints. VERY MESSY. If not readable, try messing around with cex=x
...
p + geom_text(aes(size=cl0, colour=species))+ggtitle("First two principle components and species") + geom_hline(yintercept=0)+geom_vline(xintercept=0) #geeft een ggplot van twee features voor twee pc's, met als label waar elke soort valt. verander colour=Genus als je genus wilt kleuren.
