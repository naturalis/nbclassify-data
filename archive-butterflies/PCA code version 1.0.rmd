...
##actual begin here
````{r load data}
setwd("~/Leiden/2015-2016/Stage") #change as necessary
tsv.data <- read.delim("eerste dataset 28 april.tsv")
attach(tsv.data)
...
bartlett.test(tsv.data[3:1001]) #check for variances, be sure to remove 
...
summary(MV.Dim) #importance of components of PCA.  the first x PCA's are 75% of all variance.
screeplot(MV.Dim)
````
...
MV.Dim$rotation[,1:104] #check here what are the most important components for the different principle components. Gonna have to find a way to make this more readable. This is essientially what we want though. Afterwards check what the most important features are and check below for correlations.
...
rcorr(as.matrix(tsv.matrix[,3:1001]), type=c("spearman")) #shows data-matrix of correlations between features (mcor) AND checks for p-value of correlations. if x<0.05, check if the correlation is high. If yes, you might wanna remove one of the two values.
