---
title: "Script for 150 butterfly clusters set 1"
author: "Wim"
...
##actual begin here
````{r load data}
setwd("~/Leiden/2015-2016/Stage/PCA spul/Set 1/butterfly_150_files") #change as necessary
#import data from server here!
tsv.data.orig <- read.csv("BagOfWords150.csv",sep = ";")
...
bartlett.test(tsv.data.orig[11:160]) #check for variances, skipping first ten (because they're factors). if p<0.05 variance differs greatly and we need to set cor to TRUE in the prcomp.
...
summary(mv.dim.orig) #importance of components of PCA.  the first x PCA's are 75% of all variance. Here it's the first 18.
...
mv.dim.orig$rotation[,1:18] #check here what are the most important components for the different principle components. Gonna have to find a way to make this more readable. This is essientially what we want though. Afterwards check what the most important features are and check below for correlations.
...
rcorr(as.matrix(tsv.matrix[,11:158]), type=c("spearman")) #shows data-matrix of correlations between features (mcor) AND checks for p-value of correlations. if x<0.05, check if the correlation is high. If yes, you might wanna remove one of the two values.
...
writeWebGL(dir="C:\\Users\\wim_v\\Documents\\Leiden\\2015-2016\\Stage\\PCA spul\\Set 1\\butterfly_150_files", width=700, filename=file.path("C:\\Users\\wim_v\\Documents\\Leiden\\2015-2016\\Stage\\PCA spul\\Set 1\\butterfly_150_files", "3dplot_150_set1.html")); browseURL(file.path("C:\\Users\\wim_v\\Documents\\Leiden\\2015-2016\\Stage\\PCA spul\\Set 1\\butterfly_150_files", '3dplot_150_set1.html'))
````
