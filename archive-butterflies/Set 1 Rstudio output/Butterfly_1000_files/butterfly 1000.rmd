---
title: "Script for 1000 butterfly clusters set 1"
author: "Wim"
date: "21 augustus 2016"
output:
  html_document:
    toc: yes
  word_document:
...
##actual begin here
````{r load data}
setwd("~/Leiden/2015-2016/Stage/PCA spul/Set 1/butterfly_1000_files") #change as necessary
#import data from server here!
tsv.data.orig <- read.csv("BagOfWords_1000_set1.csv",sep = ";")
...
bartlett.test(tsv.data.orig[11:1007]) #check for variances, skipping first ten (because they're factors). if p<0.05 variance differs greatly and we need to set cor to TRUE in the prcomp.
...
summary(mv.dim.orig) #importance of components of PCA.  the first x PCA's are 75% of all variance. Here it's the first 104.
...
mv.dim.orig$rotation[,1:104] #check here what are the most important components for the different principle components. Gonna have to find a way to make this more readable. This is essientially what we want though. Afterwards check what the most important features are and check below for correlations.
...
rcorr(as.matrix(tsv.matrix[,11:1007]), type=c("spearman")) #shows data-matrix of correlations between features (mcor) AND checks for p-value of correlations. if x<0.05, check if the correlation is high. If yes, you might wanna remove one of the two values.

library(corrgram) #Be sure to have corrgram installed first
corrgram(tsv.data.orig[,11:21], lower.panel = panel.ellipse,upper.panel = panel.conf,diag.panel = panel.density, cor.method = "spearman" )
...
pairs(tsv.data.orig[,11:21], main= "butterfly features", pch=21, col=species) #another way to show correlations. stick to corrgram, it's probably better.
````
##Plotting data
...
pairs(mv.dim.orig$x[, 1:12], pch = 21, col=species) #compares relationships between PC's. 19 maximum.
...
p + geom_text(aes(size=2, colour=Genus))+ggtitle("First two principle components and Genus") + geom_hline(yintercept=0)+geom_vline(xintercept=0) #geeft een ggplot van twee features voor twee pc's, met als label waar elke soort valt. verander colour=Genus als je genus wilt kleuren.
...
writeWebGL(dir="C:\\Users\\wim_v\\Documents\\Leiden\\2015-2016\\Stage\\PCA spul\\Set 1\\butterfly_1000_files", width=700, filename=file.path("C:\\Users\\wim_v\\Documents\\Leiden\\2015-2016\\Stage\\PCA spul\\Set 1\\butterfly_1000_files", "3dplot_1000_set1.html")); browseURL(file.path("C:\\Users\\wim_v\\Documents\\Leiden\\2015-2016\\Stage\\PCA spul\\Set 1\\butterfly_1000_files", '3dplot_1000_set1.html'))
````
